...
#### GLOSARIO DE LAS PUERTAS CUÁNTICAS (saltar comentario si no interesa):
    # Las puertas cuánticas son operadores unitarios (de unidades) que actúan sobre los estados cuánticos.
    ...
    # los estados base son '0' y '1'. El 'colapso probable' puede trabajarse determinando la entropía en un caso específico.
    # Con una determinación de entropía, puede haber una tendencia a colapsar a un estado base específico.
...
print(f"Estado inicial: {estado_actual}")
    ...
    # que el programador aplique una 'forma de colapso', como 'break', aunque ésta, si bien es típica sobre todo en juniors, 
    # es muy distinta a la de la mecánica cuántica, y también es muy forzada o disruptiva para proyectos grandes
                        # [dado que significaría problemas potenciales de escalabilidad].
        ### El bucle es potencialmente infinito, eso forma parte de su formalización matemática, pero evidentemente en ingeniería se aplican
        ### 'formas de colapso' (como 'break', u otras más elegantes, sobre todo pensando en desarrollo de software y arquitectura escalable).
    ...
    # superposición. Cuando colapsa, simplemente está determinado entre '0' y '1'. 
while not colapsado:
    iteracion += 1
    print(f"\n=== Iteración {iteracion} ===")
    ...
    if medicion_decision < 0.2:
        print("\n>> Momento de la verdad: Se procede a la medición cuántica, determinando el sistema.")
        resultado = medir_estado(estado_actual)
        print(f"Medición: ¡El sistema colapsa al estado {resultado}!")
        colapsado = True
    else:
        print("\n>> Medición postergada: El sistema continúa en incertidumbre... (sonidos de suspenso)")
    
print("\nEl proceso de medición ha finalizado. Fin de la simulación, y así se resuelve la incertidumbre... (Heisenberg mira orgulloso desde la distancia)")